...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    @staticmethod
    def _encode_safety_evidence(evidence_metadata: Any) -> Optional[str]:
        """Encode bounded safety metadata without accepting transcript blobs."""
        if not isinstance(evidence_metadata, dict):
            return None
        try:
            encoded = json.dumps(evidence_metadata, ensure_ascii=False, separators=(",", ":"))
        except (TypeError, ValueError):
            logger.warning("Ignoring non-serializable safety evidence metadata")
            return None
        # The finalization gate stores descriptors, never raw terminal or
        # reviewer output. Keep a hard size cap as defence in depth.
        if len(encoded.encode("utf-8")) > 8192:
            logger.warning("Ignoring oversized safety evidence metadata")
            return None
        return encoded
    ...
    def append_message(
        ...
        evidence_json = self._encode_safety_evidence(evidence_metadata)
        ...
        def _do(conn):
            ...
            cursor = conn.execute(
                """INSERT INTO messages (session_id, role, content, tool_call_id,
                   ...
                   model, billing_provider, operational_status, evidence_json)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    session_id,
                    role,
                    ...
                    _scrub_surrogates(model) if role == "assistant" and isinstance(model, str) else None,
                    _scrub_surrogates(billing_provider) if role == "assistant" and isinstance(billing_provider, str) else None,
                    operational_status if role == "assistant" and operational_status in {"verified", "unverified", "blocked"} else None,
                    evidence_json if role == "assistant" else None,
                ),
            )
    ...
    def get_latest_safety_record(self, session_id: str) -> Optional[Dict[str, Any]]:
        """Return the most recent forward safety record without replaying it.

        This deliberately bypasses conversation projection so policy metadata
        never enters a provider-facing history message.
        """
        if not session_id:
            return None
        with self._read_ctx() as conn:
            row = conn.execute(
                "SELECT id, content, timestamp, model, billing_provider, operational_status, evidence_json "
                "FROM messages WHERE session_id = ? AND active = 1 "
                "AND operational_status IS NOT NULL ORDER BY id DESC LIMIT 1",
                (session_id,),
            ).fetchone()
        if row is None:
            return None
        metadata = self._decode_display_metadata(row["evidence_json"])
        return {
            "id": row["id"],
            "content": self._decode_content(row["content"]),
        ...
        for msg in messages:
            ...
            cur = conn.execute(
                ...
                (
                    ...
                    _scrub_surrogates(msg.get("model")) if role == "assistant" and isinstance(msg.get("model"), str) else None,
                    _scrub_surrogates(msg.get("billing_provider")) if role == "assistant" and isinstance(msg.get("billing_provider"), str) else None,
                    msg.get("operational_status") if role == "assistant" and msg.get("operational_status") in {"verified", "unverified", "blocked"} else None,
                    self._encode_safety_evidence(msg.get("evidence_metadata")) if role == "assistant" else None,
                ),
            )
